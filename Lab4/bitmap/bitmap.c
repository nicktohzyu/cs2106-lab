...
long search_map(unsigned char *bitmap, int len, long num_zeroes) {
    int start = 0;
    while (start + num_zeroes < len * 8) {
        for (int j = 0; j < num_zeroes; j++) {
            int index = start + j; 
            int byte_index = ceil(index / 8);
            unsigned char mask = (unsigned char) pow(2, 7 - (index % 8));
...
void set_map(unsigned char *map, long start, long length, int value) {
    for (int i = start; i < start + length; i++) {
        int byte_index = ceil(i/8);
        unsigned char mask;
        if (value == 0) {
            mask = (unsigned char) (255 - pow(2, 7 - (i % 8)));
            map[byte_index] = map[byte_index] & mask;
        } else {
            mask = (unsigned char) pow(2, 7 - (i % 8));
            map[byte_index] = map[byte_index] | mask;
        }
    }
}
