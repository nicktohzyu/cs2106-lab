...
int *largest, *smallest; //arrays of size NUM_PROCESSES
int largest_shmid, smallest_shmid;

int main() {
    int vect[VECT_SIZE];
    int pid;

    // initialize shared memory for largest and smallest arrays
    largest_shmid = shmget(IPC_PRIVATE, sizeof(int) * NUM_PROCESSES, IPC_CREAT | 0600);
    smallest_shmid = shmget(IPC_PRIVATE, sizeof(int) * NUM_PROCESSES, IPC_CREAT | 0600);
